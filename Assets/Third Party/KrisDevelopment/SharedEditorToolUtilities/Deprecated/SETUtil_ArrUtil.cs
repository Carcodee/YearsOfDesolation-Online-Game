...
namespace SETUtil.Deprecated
{
	public static class ArrUtil
	{
		public static int Resize<T>(ref T[] arr, int addSize)
		{
			...
			{
				U.Debug.Log("[ArrUtil.Resize ERROR] Cannot resize to a negative number!");
				return arr.Length;
			}
		...
		public static void RemoveElement<T>(ref T[] arr, int? id = null)
		{
			...
			{
				U.Debug.Log("[ArrUtil.RemoveElement ERROR] id " + id + " outside array bounds!");
				return;
			}
		...
		public static void Swap<T>(ref T[] arr, int a, int b)
		{
			...
			{
				U.Debug.Log("[SETUtil.ArrUtil.Swap ERROR] Array index out of range 0 - " + arr.Length + ", aborting.");
				return;
			}
