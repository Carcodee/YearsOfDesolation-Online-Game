...
namespace SETUtil.Extend
{
	public static class ExtendUtil
	{
		...
		/// <summary>
		/// Adds Unity component if it is missing on the target object
		/// </summary>
		public static T GetOrAdd<T>(this U.Component obj) where T : U.Component
		{
			return GetOrAdd<T>(obj.gameObject);
		}

		/// <summary>
		/// Adds Unity component if it is missing on the target object
		/// </summary>
		public static T GetOrAdd<T>(this U.GameObject obj) where T : U.Component
		{
			var e = obj.GetComponent<T>();
			if (!e)
			{
				e = obj.gameObject.AddComponent<T>();
			}
		...
		public static E.SerializedProperty FindDeepProperty(this E.SerializedObject so, string path)
		{
			...
			string[] _pathArray = StringUtil.SplitPath(path);
