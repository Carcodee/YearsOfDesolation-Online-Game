...
namespace SETUtil
{
	...
	/// <summary>
	/// User interface and usability utilities
	/// </summary>
	public static class EditorUtil
	{
		...
		{
			U.Debug.Log(msg);
			if(debugLog != null) debugLog.Invoke(msg);
		}
		...
		/// </summary>
		public static List<T> ArrayFieldGUI<T>(List<T> list, ArrayFieldOption option = ArrayFieldOption.Default, Func<T> createNewElement = null)
		{
			var _listCopy = list != null ? new List<T>(list) : new List<T>();
			Func<T> _createNewElement = createNewElement != null ? createNewElement : () => { return default(T); };
			...
			if (!option.ContainsFlag(ArrayFieldOption.FixedSize)) {
				if (Gl.Button("Add Element")) {
					_listCopy.Add(_createNewElement());
				}
			}
		...
		/// <summary>
		/// Draws text input field with a select folder button (in edtior)
		/// </summary>
        public static string DrawPathInput(string label, string path, bool file, bool saveMode, string fileExtension = "")
        {
            var _path = path;
            if (string.IsNullOrEmpty(_path))
			{
				var _defaultPath = U.Application.dataPath;
				_path = _defaultPath;
            }

			U.GUILayout.BeginHorizontal();
			{
				U.GUILayout.Label(label, U.GUILayout.Width(147));
#if UNITY_EDITOR
				_path = E.EditorGUILayout.DelayedTextField(_path);

				if (Gl.Button("...", Gl.ExpandWidth(false)))
				{
					_path = saveMode ? (file ? E.EditorUtility.SaveFilePanel(label, _path, "file", fileExtension ?? "") : E.EditorUtility.SaveFolderPanel(label, _path, "")) : 
						(file ? E.EditorUtility.OpenFilePanel(label, _path, fileExtension ?? "") : E.EditorUtility.OpenFolderPanel(label, _path, ""));
				}
#else
				_path = U.GUILayout.TextField(_path);
#endif
			}
			U.GUILayout.EndHorizontal();
			return _path;
        }
