...
namespace BeautifyEffect {
    ...
    public class FrameBrowser : EditorWindow {
        ...
        void ClearBackground() {
            Shader.SetGlobalTexture(Beautify.ShaderParams.LUTPreview, Texture2D.whiteTexture);
        }
        ...
        private void OnGUI() {
            ...
            if (GUILayout.Button("Capture SceneView")) {
                RequestCapture(UnityEngine.CameraType.SceneView);
            }
            if (GUILayout.Button("Capture GameView")) {
                RequestCapture(UnityEngine.CameraType.Game);
            }
            ...
            EditorGUILayout.HelpBox("Click on the name of a Frame to toggle it on/off.\nPlease make sure that Frame and Frame Mask features are enabled in Beautify Shader Options.", MessageType.Info);
                            ...
                            for (int col = 0; col < columnCount; col++) {
                                if (matIndex < c) {
                                    ...
                                    if (frameEntry.mat != null) {
                                        ...
                                        frameEntry.mat.SetTexture(Beautify.ShaderParams.FrameMaskTexture, frameEntry.frameMask);
                                        EditorGUI.DrawPreviewTexture(rect, wt, frameEntry.mat);
                                        rect.y += rowHeight;
                                        rect.height = 15;
                                        string frameName;
                                        if (b.frame && b.frameMask == frameEntry.frameMask) {
        ...
        void RequestCapture(UnityEngine.CameraType cameraType) {

            Beautify b = Beautify.instance;
            if (b == null) {
                Debug.LogError("Beautify not found. It's requred for the Frame Browser functionality.");
                return;
            }
